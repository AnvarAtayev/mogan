...
(texmacs-module (convert markdown tmmarkdown)
  (:use (link ref-markup) (convert markdown markdown-smart-ref)
        (convert markdown markdown-utils) (convert tools tmtable)))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; S7 compatibility
;; string-capitalize is not available in S7; provide a simple implementation.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(define (string-capitalize s)
  "Capitalize the first character of string s (S7 compat for Guile's string-capitalize)"
  (if (string-null? s) s
...
;; live TeXmacs session, so we cannot use TeXmacs' built-in counter groups.
;; Counters are tuples of (value "label") so they can be deactivated when
;; interspersing unnumbered and numbered sections. Each counter has at most
;; one parent (no counter groups).
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(define (counter-parents which)
  (cdr (or (ahash-ref counters which) '(()))))
...
(define (counter-children which)
  (list-filter
    (map (lambda (entry)
           (let ((key (car entry))
                 (val (cdr entry)))
             (if (member which (cdr val)) key '())))
         (ahash-table->list counters))
    nnull?))

(define (counter-value which)
  (caar (or (ahash-ref counters which) (list (cons 0 "")))))

(define (counter-label which)
  (cdar (or (ahash-ref counters which) (list (cons 0 "")))))

(define (counter-values which)
  "List of values of counter and parents (farthest ancestor last)"
  ; Intentionally removes zeros: a subsection without a parent section
  ; should number as "1" not "0.1".
   (list-filter
...
(define (parse-with x)
  ; Recursively strips (with "prop" "val" ... content) pairs, mapping known
  ; font properties to markdown styles and discarding unknown ones.
  ; The base case (with content) has tm-length 1.
...
(for-each (lambda (l) (apply (cut ahash-set! conversion-hash <> <>) l))
          (list
      (list 'abstract keep)
...
(tm-define (texmacs->markdown x)
  ; NOTE: body must be computed before (ahash-table->list labels) so that
  ; texmacs->markdown* has a chance to populate the labels hash table.
  ; In S7, quasiquote evaluates unquoted forms left-to-right, so we separate
  ; the steps explicitly rather than relying on quasiquote ordering.
